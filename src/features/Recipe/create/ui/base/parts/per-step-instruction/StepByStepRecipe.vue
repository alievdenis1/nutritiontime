	...
	<div class="pt-[20px]">
		...
		>
			...
			<VAddPhoto
				...
				:on-image-uploaded="(imageUrl) => onImageUploaded(index, imageUrl)"
...
const { model } = useBaseRecipeCreationContext()
...
function onImageUploaded(stepIndex: number, imageUrl: string | null) {
	const exactStep = model.steps[stepIndex]

	if (exactStep) {
		exactStep.image = imageUrl
	}
}
</script>
