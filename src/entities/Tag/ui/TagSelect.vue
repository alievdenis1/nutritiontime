			...
			<div>
				<div class="tag-grid">
					<div
						...
						:class="{
							'has-active-tag': rowTags.some(tag => selectedTagsSet.has(tag.id)),
					...
					>
						...
						>
							...
							>
								...
								<span
									v-if="selectedTagsSet.has(tag.id)"
									class="tag-close"
...
const chunkedTagsByCategories = computed(() => {
	return tagsByCategories.value.reduce((acc, category) => {
		acc.push({
			...category,
