...
const CLICKER_CONFIG = {
  ...
  rapidClick: {
    threshold: 12,              // Порог для определения быстрого клика
  ...
  shake: {
    threshold: 10,             // Порог ускорения для определения тряски
  ...
  catEffect: {
    ...
    brightnessFactor: {
      min: 0.5,                // Минимальная яркость изображения при быстром клике
  ...
  sound: {
    threshold: 0.9,            // Порог громкости для определения крика (0-1)
